...
{
	if (is_writable(ROOT.'spdo') !== true) {
		@chmod(ROOT.'spdo', 0775);
	}
	if (is_writable(ROOT.'spdo') === false) {
		trigger_error("No Writable dir : ".ROOT."spdo", E_USER_ERROR);
	}
	$dirFile = ROOT.'spdo'.DS.'config.pdo.php';
	if (is_file($dirFile)) {
		@chmod($dirFile, 0777);
