...
class Gallery extends AdminPages
{
    ...
    public function index ()
    {
        ...
        foreach ($a['screen'] as $key => $value) {
            $a['screen'][$key]->nameCat = $this->models->getNameCatPrimary($value->cat_id)->name;
            $a['screen'][$key]->nameSecCat = $this->models->getNameCat($value->cat_id)->name;
        }
        $this->set($a);
    ...
    public function sendnew ()
    {
        ...
        if (ctype_alnum($_POST['id_cat']) === false) {
            $array = array(
                'type' => 'error',
        ...
        $dir = ROOT . DS . 'uploads' . DS . 'gallery'. DS. $_POST['id_cat'] .DS;
        $dirWeb = 'uploads/gallery/'. $_POST['id_cat'] .'/';
    ...
    public function sendedit ()
    {
        if (is_numeric($_POST['id']) === true) {
            ...
            $dir = ROOT . DS . 'uploads' . DS . 'gallery' . DS . $cat_id . DS;
            $dirWeb = 'uploads/gallery/'.$cat_id.'/';
    ...
    public function editcat ()
    {
        ...
        if (is_numeric($this->data[2]) === true) {
            $id = (int) $this->data[2];
            $a['cat']    = $this->models->getcatForId ($id);
            $a['groups'] = $this->models->groups ();
            $this->set($a);
        } else {
            $array = array(
    ...
    public function sendnewsubcat ()
    {
        ...
        $data['main_id']         = ctype_alnum($_POST['id_cat']) ? $_POST['id_cat'] : false;
        $data['cat_id']          = strtoupper(Common::randomString(16));

        $dirWeb = 'uploads/gallery/'. $data['cat_id'] .DS;
