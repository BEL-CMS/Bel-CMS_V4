...
class Registration extends AdminPages
{
    ...
    public function edit ()
    {
        ...
        if ($id == false or $id == 0) {
            ...
            #######################################################
            $b = array();
            $teams = $this->models->getTeams ();
            foreach ($teams as $key => $value) {
                $b['team'][$key]['name']   = $value->name;
                $b['team'][$key]['active'] = $this->models->getUserTeam ($this->data[2], $value->id);
                $b['team'][$key]['id']   = $value->id;
            }
            $this->set($b);
            #######################################################
            $c['rank'] = $this->models->getRank();
            $this->set($c);
            #######################################################
            $d['hash_key'] = $this->data[2];
            $this->set($d);
            #######################################################
            $this->render('edit');
        } else {
    ...
    {
        #########################################
        $getGames = $this->models->getTeams ();
        #########################################
        if (User::ifUserExist($_POST['hash_key'])) {
            if (isset($_POST['teams'])) {
                foreach ($getGames as $key => $value) {
                    if (array_key_exists($value->id, $_POST['teams'])) {
                        $rank = $_POST['rank'][$value->id];
                        $testGame = $this->models->getTestGames ($_POST['hash_key'], $value->id);
                        if ($testGame === true) {
                            $this->models->addNewGame ($_POST['hash_key'],$value->id, $rank);
                        } else {
                            $this->models->updateGameRank ($_POST['hash_key'], $value->id, $rank);
                        }
                    } else {
                        $this->models->deleteGameUser ($_POST['hash_key'],$value->id);
                    }
                }
                #########################################
                $array = array(
