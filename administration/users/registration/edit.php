            ...
            <div class="row mt-4 justify-content-center">
                ...
                <div class="col-md-9">
                    <div class="tab-content" id="v-pills-tabContent">
                        ...
                        <div class="tab-pane" id="main-game" role="tabpanel" aria-labelledby="main-game-tab" tabindex="0">
                            <form action="registration/updateTeam?admin&option=users" method="post">
                                <div class="row">
                                    <div class="col-12">
                                        <?php
                                        foreach ($team as $key => $value):
                                            $checked = ($value['active'] == 1) ? 'checked' : null;
                                        ?>
                                        <div class="row">
                                            <div class="col-6">
                                                <div class="form-check form-switch form-switch-info">
                                                    <input class="form-check-input" <?= $checked; ?> type="checkbox" role="switch" id="SwitchCheck<?= $key; ?>" name="teams[<?= $value['id'] ?>]" value="1">
                                                    <label class="form-check-label" for="SwitchCheck<?= $key; ?>"><?= $value['name']; ?></label>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <select class="form-select mb-4" name="rank[<?= $value['id'] ?>]">
                                                    <option value="0" selected>Choisir le grade.</option>
                                                <?php
                                                foreach ($rank as $keyRank => $valueRank):
                                                ?>
                                                    <option value="<?= $valueRank->id; ?>"><?= $valueRank->name; ?></option>
                                                <?php
                                                endforeach;
                                                ?>
                                                </select>
