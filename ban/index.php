...
final class Ban
{
	...
	public function positive ()
	{
		...
		if (self::getBan() === true) {
			...
			if (CoreUser::isLogged ()) {
				if ($this->author == $_SESSION['USER']->user->hash_key) {
					$ban  = new \DateTimeImmutable($this->endban);
					$this->countDate = $ban->format('Y/m/d H:i:s');
					$diff = $this->currentDate->diff($ban);
					if ($diff->invert == 0) { 
	...
	# Check in change IP
	#########################################
	private function checkChangeIP ()
	{
	...
	private function getUserInfo ()
	{
		if (self::isLogged() === true) {
			$user = CoreUser::getInfosUserAll($_SESSION['USER']->user->hash_key);
			if ($user !== false) {
	...
	{
		if (CoreUser::isLogged () and CoreUser::ifUserExist ($_SESSION['USER']->user->hash_key)) {
			return (bool) true;
		} else {
			return (bool) false;
		}
	...
	public static function addBan ($author = null,$ip = null, $email = null, $endban = null, $timeban = null, $reason = null)
	{
		...
		$insert['reason']   = empty($reason) ? constant('Vous avez été banni') : $reason;
