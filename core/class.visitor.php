...
final class Visitors
{
		...
		if (!empty($json)) {
			...
			if (User::isLogged() === true) {
				...
				if (Common::isBot($_SERVER["HTTP_USER_AGENT"])) {
					...
					$this->visitedUser = User::isLogged() === true ? $_SESSION['USER']->user->hash_key : constant('VISITOR');
				}
			}
		}
		$dispatcher = new Route;
		if (count($dispatcher->link) != 0 AND isset($dispatcher->link[0])) {
			$this->visitor_page = Common::VarSecure($dispatcher->link[0], null);
		} else {
			$this->visitor_page = '';
		}
		self::delStats();
	...
	private function insertBdd () 
	{
		...
		$insert['visitor_page']     = $this->visitor_page;
	...
	public static function getVisitorConnected () {
		...
		$sql->where(array(
			'name'  => 'visitor_date',
			'value' => date('Y-m-d H:i:s', strtotime('-1 min')),
			'op'    => ' >= '
		));
		$sql->queryAll();
		$data   = $sql->data;
